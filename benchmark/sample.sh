#!/bin/bash
# collects samples
# run from the project root with the makefile.
...
function sample_smt {
	...
	sed -i \
		...
		-e "s/<t1_command>/benchmark\/zsim\/$benchmark/" \
		-e "s/<t2_command>/benchmark\/zsim\/$benchmark/" \
		-e "s/<t1_rob>/$rob1/" \
		-e "s/<t2_rob>/$rob2/" \
		$sample_cfg
...
function sample_ooo {
	sed -i \
		-e "s/<dcache_size>/$dcache_size/" \
		-e "s/<icache_size>/$icache_size/" \
		-e "s/<command>/benchmark\/zsim\/$benchmark/" \
		$sample_cfg
