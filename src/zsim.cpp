...
KNOB < INT32 > KnobProcIdx(KNOB_MODE_WRITEONCE, "pintool", "procIdx", "0", "zsim process idx (internal)");
...
VOID PIN_FAST_ANALYSIS_CALL IndirectRecordBranch(THREADID tid, ADDRINT branchPc, BOOL taken, ADDRINT takenNpc,
						 ADDRINT notTakenNpc) {
...
VOID FFITrackNFFInterval() {
	...
	zinfo->eventQueue->
	    insert(makeAdaptiveEvent(ffiGet, ffiFire, 0, ffiInstrsLimit - ffiInstrsDone, MAX_IPC * zinfo->phaseLength));
...
VOID FFIInit() {
	...
	if (!ffiPoints.empty()) {
		if (zinfo->ffReinstrument)
			panic("FFI and reinstrumenting on FF switches are incompatible");
		ffiEnabled = true;
		ffiPoint = 0;
		ffiInstrsDone = 0;
...
static const InstrFuncPtrs joinPtrs =
    { JoinAndLoadSingle, JoinAndStoreSingle, JoinAndBasicBlock, JoinAndRecordBranch, JoinAndPredLoadSingle,
JoinAndPredStoreSingle, FPTR_JOIN };
static const InstrFuncPtrs nopPtrs =
    { NOPLoadStoreSingle, NOPLoadStoreSingle, NOPBasicBlock, NOPRecordBranch, NOPPredLoadStoreSingle,
NOPPredLoadStoreSingle, FPTR_NOP };
static const InstrFuncPtrs retryPtrs =
    { NOPLoadStoreSingle, NOPLoadStoreSingle, NOPBasicBlock, NOPRecordBranch, NOPPredLoadStoreSingle,
NOPPredLoadStoreSingle, FPTR_RETRY };
static const InstrFuncPtrs ffPtrs =
    { NOPLoadStoreSingle, NOPLoadStoreSingle, FFBasicBlock, NOPRecordBranch, NOPPredLoadStoreSingle,
NOPPredLoadStoreSingle, FPTR_NOP };

static const InstrFuncPtrs ffiPtrs =
    { NOPLoadStoreSingle, NOPLoadStoreSingle, FFIBasicBlock, NOPRecordBranch, NOPPredLoadStoreSingle,
NOPPredLoadStoreSingle, FPTR_NOP };
static const InstrFuncPtrs ffiEntryPtrs =
    { NOPLoadStoreSingle, NOPLoadStoreSingle, FFIEntryBasicBlock, NOPRecordBranch, NOPPredLoadStoreSingle,
NOPPredLoadStoreSingle, FPTR_NOP };
...
VOID EndOfPhaseActions() {
	...
	if (unlikely(zinfo->globalSyncedFFProcs)) {
		...
		zinfo->profSimTime->transition(PROF_FF);
		while (zinfo->globalSyncedFFProcs)
			usleep(20 * 1000);
		zinfo->profSimTime->transition(PROF_WEAVE);
...
VOID Instruction(INS ins) {
	...
	if (!procTreeNode->isInFastForward() || !zinfo->ffReinstrument) {
		...
		if (INS_IsMemoryRead(ins)) {
			if (!INS_IsPredicated(ins)) {
				...
				INS_InsertCall(ins, IPOINT_BEFORE, PredLoadFuncPtr, IARG_FAST_ANALYSIS_CALL,
					       IARG_THREAD_ID, IARG_MEMORYREAD_EA, IARG_EXECUTING, IARG_END);
			}
		}
		...
		if (INS_HasMemoryRead2(ins)) {
			if (!INS_IsPredicated(ins)) {
				...
				INS_InsertCall(ins, IPOINT_BEFORE, PredLoadFuncPtr, IARG_FAST_ANALYSIS_CALL,
					       IARG_THREAD_ID, IARG_MEMORYREAD2_EA, IARG_EXECUTING, IARG_END);
			}
		}

		if (INS_IsMemoryWrite(ins)) {
			if (!INS_IsPredicated(ins)) {
				INS_InsertCall(ins, IPOINT_BEFORE, StoreFuncPtr, IARG_FAST_ANALYSIS_CALL,
					       IARG_THREAD_ID, IARG_MEMORYWRITE_EA, IARG_END);
			} else {
				INS_InsertCall(ins, IPOINT_BEFORE, PredStoreFuncPtr, IARG_FAST_ANALYSIS_CALL,
					       IARG_THREAD_ID, IARG_MEMORYWRITE_EA, IARG_EXECUTING, IARG_END);
			}
		}
		// Instrument only conditional branches
		if (INS_Category(ins) == XED_CATEGORY_COND_BR) {
			INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR) IndirectRecordBranch, IARG_FAST_ANALYSIS_CALL,
				       IARG_THREAD_ID, IARG_INST_PTR, IARG_BRANCH_TAKEN, IARG_BRANCH_TARGET_ADDR,
	...
	if (INS_Opcode(ins) == XED_ICLASS_CPUID) {
		...
		INS_InsertCall(ins, IPOINT_AFTER, (AFUNPTR) FakeCPUIDPost, IARG_THREAD_ID, IARG_REG_REFERENCE, REG_EAX,
			       IARG_REG_REFERENCE, REG_EBX, IARG_REG_REFERENCE, REG_ECX, IARG_REG_REFERENCE, REG_EDX,
			       IARG_END);
	}
...
VOID VdsoInstrument(INS ins) {
	...
	if (unlikely(insAddr >= vdsoStart && insAddr < vdsoEnd)) {
		if (vdsoEntryMap.find(insAddr) != vdsoEntryMap.end()) {
			...
			INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR) VdsoEntryPoint, IARG_THREAD_ID, IARG_UINT32,
				       (uint32_t) func, IARG_REG_VALUE, REG_RDI, IARG_REG_VALUE, REG_RSI, IARG_END);
...
BOOL FollowChild(CHILD_PROCESS childProcess, VOID * userData) {
	...
	if (activeCount > 1)
		warn("exec() of a multithreaded process! (%d live threads)", activeCount);
...
VOID SimEnd() {
 ...
 for (AccessTraceWriter * t:*(zinfo->traceWriters))
			t->dump(false);	// flushes trace writer
	 ...
	 case ZSIM_MAGIC_OP_REGISTER_THREAD:
		 if (!zinfo->registerThreads) {
			 ...
			 if (fPtrs[tid].type == FPTR_NOP) {
				 ...
				 warn("Thread %d: Treating REGISTER_THREAD magic op as NOP, thread already registered",
				      tid);
			 }
		 }
		 return;
...
VOID FakeCPUIDPost(THREADID tid, ADDRINT * eax, ADDRINT * ebx, ADDRINT * ecx, ADDRINT * edx) {
	...
	CpuIdRecord *pos =
	    std::lower_bound(cpuid_core2, cpuid_core2 + (sizeof(cpuid_core2) / sizeof(CpuIdRecord)), val);
	if (pos->eaxIn > eaxIn) {
	...
	if (eaxIn == 4) {
		...
		// Overflowing 6 bits?
		if (zinfo->numCores > 64)
			eax3126 = 63;	//looked into swarm2.csail (4P Westmere-EX, 80 HTs), it sets this to 63
...
VOID FakeRDTSCPost(THREADID tid, REG * eax, REG * edx) {
	if (fPtrs[tid].type == FPTR_NOP)
		return;		//avoid virtualizing NOP threads.
...
class SyncEvent:public Event {
 ...
 public:
	...
	// Blocks until callback() void wait() {
		futex_lock(&arrivalLock);
	}
...
VOID FFThread(VOID * arg) {
	...
	while (true) {
		...
		bool locked =
		    futex_trylock_nospin_timeout(&zinfo->ffToggleLocks[procIdx], 5 * BILLION /*5s timeout */ );
...
static EXCEPT_HANDLING_RESULT InternalExceptionHandler(THREADID tid, EXCEPTION_INFO * pExceptInfo,
						       PHYSICAL_CONTEXT * pPhysCtxt, VOID *) {
	...
	if (PIN_GetFaultyAccessAddress(pExceptInfo, &faultyAccessAddr)) {
		...
		if (fat == FAULTY_ACCESS_READ)
			faultyAccessStr = "READ ";
		else if (fat == FAULTY_ACCESS_WRITE)
			faultyAccessStr = "WRITE ";
		else if (fat == FAULTY_ACCESS_EXECUTE)
			faultyAccessStr = "EXECUTE ";

		fprintf(stderr, "%s[%d]  Caused by invalid %saccess to address 0x%lx\n", logHeader, tid,
			faultyAccessStr, faultyAccessAddr);
	}
...
int main(int argc, char *argv[]) {
	...
	if (!masterProcess)
		procTreeNode->notifyStart();	//masterProcess notifyStart is called in init() to avoid races
