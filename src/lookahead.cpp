...
namespace lookahead {
	...
	static tuple < double, uint32_t > getMaxMarginalUtility(uint32_t numPartitions, uint32_t part,
								uint32_t partAlloc, uint32_t balance,
								const PartitionMonitor & monitor) {
		double maxMu = -1.0;
...
//Utility is defined as misses saved over not having a cache
	uint64_t computePartitioningTotalUtility(uint32_t numPartitions, const uint32_t * parts,
						 const PartitionMonitor & monitor) {
	...
	void computeBestPartitioning(uint32_t numPartitions, uint32_t buckets, uint32_t minAlloc, bool * forbidden,
...
void LookaheadPartitioner::partition() {
	...
	lookahead::computeBestPartitioning(numPartitions, allocPortion * buckets, minAlloc * numPartitions,
					   forbidden, bestAllocs, monitor);

	uint64_t newUtility = lookahead::computePartitioningTotalUtility(numPartitions, bestAllocs, monitor);
	uint64_t curUtility = lookahead::computePartitioningTotalUtility(numPartitions, curAllocs, monitor);

	bool switchAllocs = newUtility > 102 * curUtility / 100;	//must be 2% better
	if (curUtility == 0)
		switchAllocs = true;	//always switch on start (this happens when we only have recorded misses)
	...
	for (uint32_t i = 0; i < numPartitions; i++)
		info("buckets[%d] = %d", i, curAllocs[i]);
#endif
