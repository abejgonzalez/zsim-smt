...
class HDF5BackendImpl:public GlobAlloc {
 private:
	...
	// Dump the stats, inorder walk void dumpWalk(Stat * s) {
		if (skipStat(s))
			return;
		...
		if (aggrStat == nullptr) {
			...
			for (uint32_t i = 1; i < aggrStat->size(); i++) {
				...
				if (otherType != childType) {
					panic
					    ("In regular aggregate %s, child %d has a different type than first child. Doesn't look regular to me!",
					     stat->name(), i);
				}
			}
 ...
 HDF5BackendImpl(const char *_filename, AggregateStat * _rootStat, size_t _bytesPerWrite, bool _skipVectors, bool _sumRegularAggregates):
	filename(_filename), rootStat(_rootStat), skipVectors(_skipVectors), sumRegularAggregates(_sumRegularAggregates)
	{
		...
		herr_t hErrVal = H5TBmake_table("stats", fileID, "stats",
						...
						recordsPerWrite
						/*chunk size, in records, might as well be our aggregation degree */ ,
						nullptr, 9 /*compression */ , nullptr);
		assert(hErrVal == 0);

		size_t bufSize = recordsPerWrite * recordSize;
		if (sumRegularAggregates)
			bufSize += recordSize;	//conservatively add space for a record. See dumpWalk(), we bleed into the buffer a bit when dumping a regular aggregate.
	...
	void dump(bool buffered) {
		...
		assert_msg(dataBuf + bufferedRecords * recordSize / sizeof(uint64_t) == curPtr,
			   "HDF5 (%s): %p + %d * %ld / %ld != %p", filename, dataBuf, bufferedRecords, recordSize,
			   sizeof(uint64_t), curPtr);
		...
		if (bufferedRecords == recordsPerWrite || !buffered) {
			...
			H5TBappend_records(fileID, "stats", bufferedRecords, recordSize, fieldOffsets, fieldSizes,
					   dataBuf);
			H5Fclose(fileID);
...
HDF5Backend::HDF5Backend(const char *filename, AggregateStat * rootStat, size_t bytesPerWrite, bool skipVectors,
			 bool sumRegularAggregates) {
