...
#define PT_CHUNKSIZE (1024*256u)	// 256K records (~6MB)
 ...
 AccessTraceReader::AccessTraceReader(std::string _fname):fname(_fname.c_str()) {
	...
	if (fid == H5I_INVALID_HID)
		panic("Could not open HDF5 file %s", fname.c_str());
	...
	if (!finished)
		panic("Trace file %s unfinished (halted simulation?)", fname.c_str());

	// Populate numRecords & numChildren
	hsize_t nPackets;
	hid_t table = H5PTopen(fid, "accs");
	if (table == H5I_INVALID_HID)
		panic("Could not open HDF5 packet table");
...
void AccessTraceReader::nextChunk() {
 ...
 AccessTraceWriter::AccessTraceWriter(g_string _fname, uint32_t numChildren):fname(_fname) {
	...
	size_t offset = 0;
	size_t size =
	    H5Tget_size(H5T_NATIVE_ULONG) * 2 + H5Tget_size(H5T_NATIVE_UINT) + H5Tget_size(H5T_NATIVE_USHORT) +
	    H5Tget_size(accType);
	...
	if (fid == H5I_INVALID_HID)
		panic("Could not create HDF5 file %s", fname.c_str());
	...
	if (table == H5I_INVALID_HID)
		panic("Could not create HDF5 dataset");
	H5Dclose(table);
