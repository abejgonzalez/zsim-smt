...
#include "zsim.h"
 ...
 SimpleCore::SimpleCore(FilterCache * _l1i, FilterCache * _l1d, g_string & _name):Core(_name), l1i(_l1i), l1d(_l1d), instrs(0), curCycle(0),
haltedCycles(0) {
}
...
void SimpleCore::initStats(AggregateStat * parentStat) {
	...
	auto x =[this] ()->uint64_t { assert(curCycle >= haltedCycles);
...
uint64_t SimpleCore::getPhaseCycles() const const {
