...
class DRAMSimAccEvent:public TimingEvent {
 ...
 public:
	 ...
	 DRAMSimAccEvent(DRAMSimMemory * _dram, bool _write, Address _addr, int32_t domain):TimingEvent(0, 0, domain),
	    dram(_dram), write(_write), addr(_addr) {
	} bool isWrite() const {
		return write;
...
DRAMSimMemory::DRAMSimMemory(string & dramTechIni, string & dramSystemIni, string & outputDir, string & traceName,
	...
	TransactionCompleteCB *read_cb =
	    new Callback < DRAMSimMemory, void, unsigned, uint64_t, uint64_t > (this,
										&DRAMSimMemory::DRAM_read_return_cb);
	TransactionCompleteCB *write_cb =
	    new Callback < DRAMSimMemory, void, unsigned, uint64_t, uint64_t > (this,
										&DRAMSimMemory::DRAM_write_return_cb);
...
uint64_t DRAMSimMemory::access(MemReq & req) {
	...
	if ((req.type != PUTS /*discard clean writebacks */ ) && zinfo->eventRecorders[req.srcId]) {
		...
		DRAMSimAccEvent *memEv =
		    new(zinfo->eventRecorders[req.srcId]) DRAMSimAccEvent(this, isWrite, addr, domain);
