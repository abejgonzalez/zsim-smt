...
class DDRMemoryAccEvent:public TimingEvent {
 ...
 public:
	 DDRMemoryAccEvent(DDRMemory * _mem, bool _isWrite, Address _addr, int32_t domain, uint32_t preDelay,
			   uint32_t postDelay)
...
deferredWrites(_deferredWrites), closedPage(_closedPage), domain(_domain), name(_name) {
	...
	for (uint32_t i = 0; i < ranksPerChannel; i++)
		banks[i].resize(banksPerRank);

	rankActWindows.resize(ranksPerChannel);
	for (uint32_t i = 0; i < ranksPerChannel; i++)
		rankActWindows[i].init(4);	// we only model FAW; for TAW (other technologies) change this to 2
	...
	if (tokens.size() != 3)
		panic("Invalid addrMapping %s, need all row/col/rank tokens separated by colons", addrMapping);
	std::reverse(tokens.begin(), tokens.end());	// want lowest bits first

	colMask = rankMask = bankMask = 0;
	uint32_t startBit = 0;
	auto computeShiftAndMask =
	    [&startBit, addrMapping] (const std::string & field, const uint32_t fieldBits, uint32_t & shift,
				      uint32_t & mask){
		if (mask)
			panic("Repeated field %s in addrMapping %s", field.c_str(), addrMapping);
		shift = startBit;
		mask = (1 << fieldBits) - 1;
		startBit += fieldBits;
	};
 for (auto t:tokens) {
		if (t == "col")
			computeShiftAndMask(t, colBits, colShift, colMask);
		else if (t == "rank")
			computeShiftAndMask(t, rankBits, rankShift, rankMask);
		else if (t == "bank")
			computeShiftAndMask(t, bankBits, bankShift, bankMask);
		else
			panic("Invalid token %s in addrMapping %s (only row/col/rank)", t.c_str(), addrMapping);
	}
...
void DDRMemory::initStats(AggregateStat * parentStat) {
	...
	profReadHits.init("rdhits", "Read row hits");
	memStats->append(&profReadHits);
	profWriteHits.init("wrhits", "Write row hits");
	memStats->append(&profWriteHits);
	...
	if (overflow) {
		...
		if (!req->prev /* first in bank */ ) {
			...
			if (nextSchedCycle > minSchedCycle)
				minSchedCycle = std::max(minSchedCycle, findMinCmdCycle(*req));
			if (nextSchedCycle > minSchedCycle) {
				if (nextSchedEvent)
					nextSchedEvent->annul();
				if (eventFreelist) {
...
void DDRMemory::queue(Request * req, uint64_t memCycle) {
	...
	auto printQ =[&](const char *id){
		info("%8ld: %s r%db%d : %s  %s", memCycle, name.c_str(), req->loc.rank, req->loc.bank,
		     (deferredWrites && req->write) ? "WQ" : "RQ", id);
		Request *pr = q.front();
		while (pr) {
		...
		*req = ovfReq;
		...
		if (!req->prev /*first in bank queue */ ) {
			...
			if (minSchedCycle > minQueuedSchedCycle)
				minSchedCycle = std::max(minQueuedSchedCycle, findMinCmdCycle(*req));
			if (minSchedCycle > minQueuedSchedCycle) {
				DEBUG("Overflowed request lowered minSchedCycle %ld -> %ld (memCycle %ld)",
				      minSchedCycle, minQueuedSchedCycle, memCycle);
...
uint64_t DDRMemory::findMinCmdCycle(const Request & r) const const {
...
uint64_t DDRMemory::trySchedule(uint64_t curCycle, uint64_t sysCycle) {
	...
	if (rdQueue.empty() && wrQueue.empty())
		return -1ul;
	if (curCycle + tCL < minRespCycle)
		return minRespCycle - tCL;	// too far ahead

	// Writes have priority if the write queue is getting full...
	bool prioWrites = (wrQueue.size() > (3 * queueDepth / 4)) || (lastCmdWasWrite
								      && wrQueue.size() > queueDepth / 4);
	...
	DEBUG("%ld : Found ready request 0x%lx %s %ld (%ld / %ld)", curCycle, r->addr, r->write ? "W" : "R",
	      r->arrivalCycle, rdQueue.size(), wrQueue.size());
	...
	if (lastCmdWasWrite && !r->write)
		minCmdCycle = std::max(minCmdCycle, minRespCycle + tWTR);
	bool rowHit = false;
	if (r->loc.row == bank.openRow && bank.open) {
		// Row buffer hit
		rowHit = true;
		...
		if (preIssued)
			bank.minPreCycle = preCycle + tRAS;
	...
	if (closedPage && !(r->next && r->next->rowHitSeq != 0))
		bank.open = false;
	bank.minPreCycle = std::max(bank.minPreCycle,	// for mixed read and write commands, minPreCycle may not be monotonic without this
