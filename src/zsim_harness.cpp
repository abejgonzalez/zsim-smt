...
int eraseChild(int pid) {
	for (int i = 0; i < MAX_CHILDREN; i++) {
		if (childInfo[i].pid == pid) {
			assert_msg(childInfo[i].status == PS_RUNNING, "i=%d pid=%d status=%d", i, pid,
				   childInfo[i].status);
...
void sigHandler(int sig) {
	if (termStatus == KILL_EM_ALL)
		return;		//a kill was already issued, avoid infinite recursion
...
/**
 * fork and kick off a new pin container.
 * register the new pin process with the Arbitration Unit.
 */
void LaunchProcess(uint32_t procIdx) {
	int cpid = fork();
	if (cpid) {		//parent
		...
		if (!aslr) {
			...
			int pers =
			    personality(((unsigned int)-1)
					/*returns current pers flags; arg is a long, hence the cast, see man */ );
...
int main(int argc, char *argv[]) {
	...
	ArbitrationUnit.printWorkingSet();

	if (numProcs == 0)
		panic("No process config found. Config file needs at least a process0 entry");
		...
		if (deadlockDetection) {
			if (simShouldAdvance) {
				...
				if (numPhases <= lastNumPhases) {
					...
					if (secsStalled > 10)
						warn("Stalled for %d secs so far", secsStalled);
				} else {
				...
				if (numPhases == lastNumPhases)
					info("Some fast-forwarding is going on, not doing deadlock detection (a: %d, ff: %d, sff: %d)", activeProcs, ffProcs, sffProcs);
				lastNumPhases = numPhases;
	...
	if (zinfo && zinfo->globalActiveProcs)
		warn("Unclean exit of %d children, termination stats were most likely not dumped",
		     zinfo->globalActiveProcs);
	exit(exitCode);
}
