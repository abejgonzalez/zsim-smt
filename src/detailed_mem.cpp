...
uint64_t MemChannelBase::CalcIntraIssueCycle(bool rowHit, uint32_t rank, MemAccessType type, uint64_t arrivalCycle,
	...
	if ((lastRefreshCycle != 0) && (refCycle >= refOverlap) && (lastAccessCycle >= lastRefreshCycle)) {
...
uint64_t MemChannelBase::LatencySimulate(Address lineAddr, uint64_t arrivalCycle, uint64_t lastPhaseCycle,
					 MemAccessType type) {
	...
	if (mParam->IsOpenRowBufPolicy()) {
		...
		if (continuous == false) {
			actIssueCycle = CalcIntraIssueCycle(rowHit, rank, type, arrivalCycle, refreshNum);
		}
	}
	...
	if (actIssueCycle == (uint64_t) - 1) {
		...
		if (continuous == true) {
			...
			rdwrStart = CalcIntraIssueCycle(rowHit, rank, type, arrivalCycle, refreshNum);
		}
	} else {
...
bool MemChannelBase::CheckContinuousAccess(uint64_t arrivalCycle, uint32_t rank, uint32_t bank, uint32_t row) {
	...
	uint64_t lastPreCycle = ranks[rank]->GetLastPreCycle(bank);
	if ((arrivalCycle < lastPreCycle) && (ranks[rank]->GetLastRow(bank) == row)) {	// w/o ACT
		return true;
	} else {
		return false;
	}
...
// Main Memory Class
MemControllerBase::MemControllerBase(g_string _memCfg, uint32_t _cacheLineSize, uint32_t _sysFreqMHz, uint32_t _domain,
				     g_string & _name) {
	name = _name;
	domain = _domain;
...
uint64_t MemControllerBase::access(MemReq & req) {
	...
	if ((req.type != PUTS) && zinfo->eventRecorders[req.srcId]) {
		...
		MemAccessEventBase *memEv = new(zinfo->eventRecorders[req.srcId])
...
void MemControllerBase::EstimatePowers(uint64_t sysCycle, bool finish) {
	...
	if (mParam->curAvgPowerReport == true) {
		...
		curPower.total =
		    curPower.burst + curPower.actPre + curPower.refresh + curPower.background + curPower.dq +
		    curPower.terminate;
	...
	if (mParam->accAvgPowerReport == true) {
		...
		accPower.total =
		    accPower.burst + accPower.actPre + accPower.refresh + accPower.background + accPower.dq +
		    accPower.terminate;
		// profile update
		for (uint32_t i = 0; i < pwCounterNum; i++)
	...
	if (mParam->accAvgPowerReport == true && finish == true) {
		info("Cummulative Average Power(mW): Total= %6ld, ActPre= %6ld, Burst= %6ld, Refresh= %6ld, BackGrnd= %6ld, ModuleDQ= %6ld, Terminate= %6ld", accPower.total, accPower.actPre, accPower.burst, accPower.refresh, accPower.background, accPower.dq, accPower.terminate);
	}
