...
void TimingEvent::produceCrossings(EventRecorder * evRec) {
	...
	auto pcLambda =[this, evRec] (TimingEvent ** childPtr) {
		...
		if (c->domain != domain)
			*childPtr = handleCrossing(c, evRec, true);
...
TimingEvent *TimingEvent::handleCrossing(TimingEvent * childEv, EventRecorder * evRec, bool unlinkChild) {
	...
	assert_msg(minStartCycle != ((uint64_t) - 1L), "Crossing domain (%d -> %d), but parent's minStartCycle is not set (my class: %s)", domain, childEv->domain, typeid(*this).name());	//we can only handle a crossing if this has been set
