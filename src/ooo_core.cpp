...
#define RF_READS_PER_CYCLE 3
 ...
 OOOCore::OOOCore(FilterCache * _l1i, FilterCache * _l1d, g_string & _name):Core(_name), l1i(_l1i), l1d(_l1d), cRec(0,
					_name) 
{
	...
	for (uint32_t i = 0; i < FWD_ENTRIES; i++)
		fwdArray[i].set((Address) (-1L), 0);
}
...
void OOOCore::initStats(AggregateStat * parentStat) {
	...
	profDecodeStalls.init("decodeStalls", "Decode stalls");
	coreStat->append(&profDecodeStalls);
...
uint64_t OOOCore::getInstrs() const const {
	return instrs;
}
uint64_t OOOCore::getPhaseCycles() const const {
	...
	for (uint32_t i = 0; i < bbl->uops; i++) {
		...
		switch (uop->type) {
		 ...
		 default:
			 ...
			 lastStoreAddrCommitCycle =
			     MAX(commitCycle, MAX(lastStoreAddrCommitCycle, lastStoreCommitCycle + uop->lat));
...
void OOOCore::LoadFunc(THREADID tid, ADDRINT addr) {
	static_cast < OOOCore * >(cores[tid])->load(addr);
}
void OOOCore::StoreFunc(THREADID tid, ADDRINT addr) {
	static_cast < OOOCore * >(cores[tid])->store(addr);
}
