...
static void TrueSleep(uint32_t usecs) {
	...
	while (req.tv_sec != 0 || req.tv_nsec != 0) {
		...
		req = rem;
		if (res != -EINTR && res != 0)
			panic("nanosleep() returned an unexpected error code %d", res);
...
void Scheduler::watchdogThreadFunc() {
	...
	while (true) {
		...
		if (lastPhase == curPhase && !fakeLeaves.empty()
		    && (fakeLeaves.front()->th->futexJoin.action != FJA_WAKE)) {
			if (++fakeLeaveStalls >= WATCHDOG_STALL_THRESHOLD) {
				...
				if (std::regex_match(GetSyscallName(fl->syscallNumber), sbRegex)) {
					...
					info("Skipping, [%d] %s @ 0x%lx | arg0 0x%lx arg1 0x%lx does not match blacklist regex (%s)", pid, GetSyscallName(fl->syscallNumber), fl->pc, fl->arg0, fl->arg1, sbRegexStr.c_str());
				}
				fakeLeaveStalls = 0;
			}
		} else {
		...
		if (lastPhase == curPhase && scheduledThreads == outQueue.size() && !sleepQueue.empty()) {
			...
			if (wakeupUsec > 10 * 1000 * 1000)
				warn("Watchdog sleeping for a long time due to long sleep, %ld secs",
	...
	while (true) {
		...
		while (wokenUp > unmatchedFutexWakeups) {
			...
			if (curNs - startNs > (2L << 31L) /* ~2s */ ) {
				futex_lock(&schedLock);
				warn("Futex wake matching failed (%d/%d) (external/ff waiters?)", unmatchedFutexWakeups,
				     wokenUp);
