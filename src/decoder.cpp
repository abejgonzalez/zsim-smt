 ...
 Decoder::Instr::Instr(INS _ins):ins(_ins), numLoads(0), numInRegs(0), numOutRegs(0), numStores(0) {
	...
	for (uint32_t op = 0; op < numOperands; op++) {
		...
		if (INS_OperandIsMemory(ins, op)) {
			...
			if (read)
				inRegs[numInRegs++] = reg;
			if (write)
				outRegs[numOutRegs++] = reg;
		}
	}
...
void Decoder::reportUnhandledCase(Instr & instr, const char *desc) {
	warn("Unhandled case: %s | %s | loads=%d stores=%d inRegs=%d %s outRegs=%d %s", desc,
	     INS_Disassemble(instr.ins).c_str(), instr.numLoads, instr.numStores, instr.numInRegs,
	     regsToString(instr.inRegs, instr.numInRegs).c_str(), instr.numOutRegs, regsToString(instr.outRegs,
												 instr.numOutRegs).
	     c_str());
}
...
void Decoder::emitExecUop(uint32_t rs0, uint32_t rs1, uint32_t rd0, uint32_t rd1, DynUopVec & uops, uint32_t lat,
			  uint8_t ports, uint8_t extraSlots) {
	DynUop uop;
	uop.clear();
	uop.rs[0] = rs0;
	uop.rs[1] = rs1;
...
void Decoder::emitXchg(Instr & instr, DynUopVec & uops) {
	if (instr.numLoads) {	// mem <-> reg
		...
		if (!INS_LockPrefix(instr.ins))
			emitFence(uops, 14);	//xchg has an implicit lock prefix (TODO: Check we don't introduce two fences...)
	} else {		// reg <-> reg
...
void Decoder::emitBasicOp(Instr & instr, DynUopVec & uops, uint32_t lat, uint8_t ports, uint8_t extraSlots,
...
void Decoder::emitChainedOp(Instr & instr, DynUopVec & uops, uint32_t numUops, uint32_t * latArray,
			    uint8_t * portsArray) {
	emitLoads(instr, uops);
	...
	emitExecUop(REG_EXEC_TEMP, srcRegs[numUops - 1], dstRegs[0], dstRegs[1], uops, latArray[numUops - 1],
		    portsArray[numUops - 1]);

	emitStores(instr, uops);
}

//Some convert ops are implemented in 2 uops, even though they could just use one given src/dst reg constraints
void Decoder::emitConvert2Op(Instr & instr, DynUopVec & uops, uint32_t lat1, uint32_t lat2, uint8_t ports1,
			     uint8_t ports2) {
...
static bool dropRegister(uint32_t targetReg, uint32_t * regs, uint32_t & numRegs) {
	for (uint32_t i = 0; i < numRegs; i++) {
		...
		if (reg == targetReg) {
			//Shift rest of regs
			for (uint32_t j = i; j < numRegs - 1; j++)
				regs[j] = regs[j + 1];
			numRegs--;
			return true;
		}
...
bool Decoder::decodeInstr(INS ins, DynUopVec & uops) {
	...
	switch (category) {
	 ...
	 case XC(ROTATE):
		 {
			 ...
			 if (opcode == XO(RCR) || opcode == XO(RCL))
				 opLat = 2;
	 ...
	 case XC(RET):
		 ...
		 emitBasicOp(instr, uops, 1, PORT_5);
		 if (opcode != XO(CALL_NEAR) && opcode != XO(RET_NEAR))
			 inaccurate = true;	//far call/ret or irets are far more complex
		 break;
...
bool Decoder::canFuse(INS ins) {
	xed_iclass_enum_t opcode = (xed_iclass_enum_t) INS_Opcode(ins);
	if (!(opcode == XO(CMP) || opcode == XO(TEST)))
		return false;
	...
	if (nextCategory != XC(COND_BR))
		return false;
	if (!INS_IsDirectBranch(nextIns))
		return false;	//according to PIN's API, this s only true for PC-rel near branches
...
BblInfo *Decoder::decodeBbl(BBL bbl, bool oooDecoding) {
	...
	if (oooDecoding) {
		...
		for (uint32_t i = 0; i < dynBbl.uops; i++)
			dynBbl.uop[i] = uopVec[i];
...
void Decoder::dumpBblProfile() {
	...
	for (uint32_t i = 0; i < numOpcodes; i++)
		approxOpcodeCount[i] = 0;
	for (uint32_t i = 0; i < bblIdx; i++) {
		if (bblApproxOpcodes[i])
 for (uint32_t & j:	*bblApproxOpcodes[i])
				approxOpcodeCount[j] += bblCount[i];
	}

	std::ofstream out("approx_instrs.stats");
	out << std::setw(16) << "Category" << std::setw(16) << "Iclass" << std::setw(32) << "Iform" << std::
	    setw(16) << "Count" << std::endl;
	for (uint32_t i = 0; i < numOpcodes; i++) {
		if (approxOpcodeCount[i]) {
			...
			out << std::setw(16) << xed_category_enum_t2str(cat) << std::
			    setw(16) << xed_iclass_enum_t2str(iclass) << std::
			    setw(32) << xed_iform_enum_t2str(iform) << std::setw(16) << approxOpcodeCount[i] << std::
			    endl;
		}
	}
