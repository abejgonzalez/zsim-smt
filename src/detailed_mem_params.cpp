...
void MemParam::LoadConfigMain(Config & cfg, uint32_t _cacheLineSize) {
	...
	if (rankCount == 0)
		panic
		    ("Illegal specs!!! Please check mc_spec.capacityMB, mc_spec.channels, mem_spec.cpacityMb and mem_spec.dataBusWidth.");
...
void MemParam::MakeConstraints(void) {
	...
	//idx=0 R->R
	constraints[0 b00] = std::max(tTrans, tCCD);

	//idx=1 R->W
	constraints[0 b01] = tCAS + tCCD / 2 + 2 - tCWD;

	//idx=2 W->R
	constraints[0 b10] = tCWD + tTrans + tWTR;

	//idx=3 W->W
	constraints[0 b11] = std::max(tCCD, tTrans);
}
