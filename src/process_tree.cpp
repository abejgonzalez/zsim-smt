...
bool ProcessTreeNode::notifyStart() {
	if (!started) {		//first start
		...
		if (procIdx) {
			if (oldActiveProcs == 0) {
				panic
				    ("Race! All processes finished before this one started, so stats have already been dumped and sim state may be partially deleted. "
		...
		if (inFastForward) {
			if (getSyncedFastForward())
				__sync_fetch_and_add(&zinfo->globalSyncedFFProcs, 1);
...
void ProcessTreeNode::exitFastForward() {
	...
	inFastForward = false;
	if (getSyncedFastForward())
		__sync_fetch_and_sub(&zinfo->globalSyncedFFProcs, 1);
...
void ProcessTreeNode::heartbeat() {
 ...
 public:
		explicit EventualStatsDumpEvent(uint64_t _p):Event(0 /*one-shot */ ), p(_p) {
			...
			DumpEventualStats(p, "heartbeats");
		}
	};
...
static void PopulateLevel(Config & config, const std::string & prefix, std::vector < ProcessTreeNode * >&globProcVector,
	...
	while (true) {
		...
		if (groupWithPrevious) {
			if (procIdx == 0)
				panic("Can't group process0 with the previous one, there is not previous process");
			assert(groupIdx > 0);
			groupIdx--;
		}

		bool startFastForwarded = config.get < bool > (p_ss.str() + ".startFastForwarded", false);
		g_string syncedFastForwardStr =
		    config.get < const char *>(p_ss.str() + ".syncedFastForward", "Multiprocess");
		...
		g_string syscallBlacklistRegex =
		    config.get < const char *>(p_ss.str() + ".syscallBlacklistRegex", ".*");
		g_vector < bool > mask;
		if (!zinfo->traceDriven) {
			mask =
			    ParseMask(config.get < const char *>(p_ss.str() + ".mask", DefaultMaskStr().c_str()),
				      zinfo->numCores);
		}		//  else leave mask empty, no cores
		g_vector < uint64_t > ffiPoints(ParseList < uint64_t >
						(config.get < const char *>(p_ss.str() + ".ffiPoints", "")));

		if (dumpInstrs) {
			if (dumpHeartbeats)
				warn("Dumping eventual stats on both heartbeats AND instructions; you won't be able to distinguish both!");
			...
			zinfo->eventQueue->
			    insert(makeAdaptiveEvent
				   (getInstrs, dumpStats, 0, dumpInstrs,
				    MAX_IPC * zinfo->phaseLength * zinfo->numCores /*all cores can be on */ ));
		}		//NOTE: trivial to do the same with cycles

		if (clockDomain >= MAX_CLOCK_DOMAINS)
			panic("Invalid clock domain %d", clockDomain);
		if (portDomain >= MAX_PORT_DOMAINS)
			panic("Invalid port domain %d", portDomain);
		...
		ProcessTreeNode *ptn =
		    new ProcessTreeNode(procIdx, groupIdx, startFastForwarded, startPaused, syncedFastForward,
					clockDomain, portDomain, dumpHeartbeats, dumpsResetHeartbeats, restarts, mask,
					ffiPoints, syscallBlacklistRegex, gpr);
...
void CreateProcessTree(Config & config) {
	ProcessTreeNode *rootNode =
	    new ProcessTreeNode(-1, -1, false, false, SFF_NEVER, 0, 0, 0, false, 0, g_vector < bool > { },
				g_vector < uint64_t > { }, g_string { }, nullptr);
	uint32_t procIdx = 0;
	uint32_t groupIdx = 0;
	...
	if (procIdx > (uint32_t) zinfo->lineSize)
		panic
	...
	for (uint32_t i = 0; i < procIdx; i++)
		zinfo->procArray[i] = globProcVector[i];
