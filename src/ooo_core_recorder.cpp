...
class OOOIssueEvent:public TimingEvent {
 ...
 public:
  OOOIssueEvent(uint32_t preDelay, uint64_t _zllStartCycle, OOOCoreRecorder * _cRec, int32_t domain = -1):TimingEvent(preDelay, 0, domain), zllStartCycle(_zllStartCycle),
	    cRec(_cRec)
	{
	} void simulate(uint64_t startCycle) {
		TRACE_MSG("Issue %ld zllStartCycle %ld startCycle %ld minStartCycle %ld", id, zllStartCycle, startCycle,
			  getMinStartCycle());
...
class OOODispatchEvent:public TimingEvent {
 ...
 public:
  OOODispatchEvent(uint64_t preDelay, uint64_t _zllStartCycle, int32_t domain = -1):TimingEvent(preDelay, 0, domain),
	    zllStartCycle(_zllStartCycle)
	{
	} void simulate(uint64_t startCycle) {
		TRACE_MSG("Dispatch %ld zllStartCycle %ld startCycle %ld minStartCycle %ld", id, zllStartCycle,
			  startCycle, getMinStartCycle());
		done(startCycle);
	}
...
class OOORespEvent:public TimingEvent {
 ...
 public:
  OOORespEvent(uint64_t preDelay, OOOCoreRecorder * _cRec, int32_t domain = -1):TimingEvent(preDelay, 0, domain),
	    cRec(_cRec) {
...
bool OOOCoreRecorder::CompareRespEvents::operator() (const FutureResponse & e1, const FutureResponse & e2)
const {
...
uint64_t OOOCoreRecorder::notifyJoin(uint64_t curCycle) {
	if (state == HALTED) {
		...
		DEBUG_MSG("[%s] Joined, was HALTED, curCycle %ld halted %ld", name.c_str(), curCycle,
			  totalHaltedCycles);
...
void OOOCoreRecorder::addIssueEvent(uint64_t evCycle) {
	...
	assert_msg(zllCycle >= lastEvProduced->zllStartCycle, "zllCycle %ld last %ld", zllCycle,
		   lastEvProduced->zllStartCycle);
	...
	while (!futureResponses.empty()) {
		...
		if (firstResp.zllStartCycle >= zllCycle)
			break;
		if (firstResp.ev) {
...
template < class T, class S, class C > S & GetPrioQueueContainer(std::priority_queue < T, S, C > &q) {
...
void OOOCoreRecorder::recordAccess(uint64_t curCycle, uint64_t dispatchCycle, uint64_t respCycle) {
	...
	if (IsGet(tr.type)) {
		...
		//Dispatch event
		OOODispatchEvent *dispEv =
		    new(eventRecorder) OOODispatchEvent( /*dispatchCycle - curCycle */ 0, dispatchCycle);
 ...
 for (FutureResponse & fr:GetPrioQueueContainer(futureResponses)) {
		...
		lastEvProduced->addChild(dDisp, eventRecorder)->addChild(dispEv, eventRecorder)->addChild(dUp,
													  eventRecorder)->
		...
		futureResponses.push( {
				     zllStartCycle, respEvent}
		);
	} else {
...
uint64_t OOOCoreRecorder::cSimStart(uint64_t curCycle) {
	...
	if (state == RUNNING) {
		...
		if (curCycle < nextPhaseCycle)
			curCycle = nextPhaseCycle;	// bring cycle up
	}
	return curCycle;
}
...
uint64_t OOOCoreRecorder::cSimEnd(uint64_t curCycle) {
	...
	assert_msg(lastEvCycle2 <= curCycle, "[%s] lec2 %ld cc %ld, state %d", name.c_str(), lastEvCycle2, curCycle,
		   state);
	if (unlikely(lastEvCycle1 > lastEvCycle2))
		panic
		    ("[%s] Contention simulation introduced a negative skew, curCycle %ld, lc1 %ld lc2 %ld, gapCycles %ld",
		     name.c_str(), curCycle, lastEvCycle1, lastEvCycle2, gapCycles);
	...
	DEBUG_MSG("[%s] curCycle %ld zllCurCycle %ld lec1 %ld lec2 %ld skew %ld", name.c_str(), curCycle,
		  curCycle - gapCycles, lastEvCycle1, lastEvCycle2, skew);
 ...
 for (FutureResponse & fr:GetPrioQueueContainer(futureResponses)) {
	...
	if (!lastEvProduced) {
		...
		assert_msg(state == DRAINING, "[%s] state %d lastEvSimulated startCycle %ld curCycle %ld", name.c_str(),
			   state, lastEvSimulatedStartCycle, curCycle);
		lastUnhaltedCycle = lastEvSimulatedStartCycle;	//the taper is a 0-delay event
		state = HALTED;
		DEBUG_MSG("[%s] lastEvSimulated reached (startCycle %ld), DRAINING -> HALTED", name.c_str(),
			  lastEvSimulatedStartCycle);
...
void OOOCoreRecorder::reportIssueEventSimulated(OOOIssueEvent * ev, uint64_t startCycle) {
	...
	if (lastEvProduced == ev)
		lastEvProduced = nullptr;
	eventRecorder.setStartSlack(startCycle - ev->zllStartCycle);
}

//Stats
uint64_t OOOCoreRecorder::getUnhaltedCycles(uint64_t curCycle) const const {
...
uint64_t OOOCoreRecorder::getContentionCycles() const const {
