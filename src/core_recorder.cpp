...
class TimingCoreEvent:public TimingEvent {
 ...
 public:
  ...
  TimingCoreEvent(uint64_t _delay, uint64_t _origStartCycle, CoreRecorder * _cRec, int32_t domain = -1):TimingEvent(0, _delay, domain), origStartCycle(_origStartCycle),
	    cRec(_cRec)
	{
...
uint64_t CoreRecorder::cSimStart(uint64_t curCycle) {
	...
	if (state == RUNNING) {
		...
		if (prevRespCycle < curCycle) {
			uint64_t delay = curCycle - prevRespCycle;
			TimingCoreEvent *ev =
			    new(eventRecorder) TimingCoreEvent(delay, prevRespCycle - gapCycles, this);
			ev->setMinStartCycle(prevRespCycle);
			prevRespEvent->addChild(ev, eventRecorder);
			prevRespEvent = ev;
			prevRespCycle = curCycle;
...
uint64_t CoreRecorder::cSimEnd(uint64_t curCycle) {
	...
	if (unlikely(lastEvCycle1 > lastEvCycle2))
		panic("[%s] Contention simulation introduced a negative skew, curCycle %ld, lc1 %ld lc2 %ld",
		      name.c_str(), curCycle, lastEvCycle1, lastEvCycle2);
	...
	if (!prevRespEvent) {
		...
		assert_msg(state == DRAINING, "[%s] state %d lastEventSimulated startCycle %ld curCycle %ld",
			   name.c_str(), state, lastEventSimulatedStartCycle, curCycle);
...
//Stats
uint64_t CoreRecorder::getUnhaltedCycles(uint64_t curCycle) const const {
...
uint64_t CoreRecorder::getContentionCycles() const const {
