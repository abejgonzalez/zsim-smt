...
#endif
		panic("Input config file %s could not be parsed, line %d, error: %s", peFile, pe.getLine(),
		      pe.getError());
	}
}
...
static uint32_t copyNonSimVars(libconfig::Setting & s1, libconfig::Setting & s2, std::string prefix) {
	...
	for (uint32_t i = 0; i < (uint32_t) s1.getLength(); i++) {
		...
		if (name[0] == '*') {
			...
			if (libconfig::Setting::Type::TypeInt == s1[i].getType())
				ns = (int)s1[i];
			else if (libconfig::Setting::Type::TypeInt64 == s1[i].getType())
				ns = (lc_int64) s1[i];
			else if (libconfig::Setting::Type::TypeBoolean == s1[i].getType())
				ns = (bool) s1[i];
			else if (libconfig::Setting::Type::TypeString == s1[i].getType())
				ns = (const char *)s1[i];
			else
...
void Config::writeAndClose(const char *outFile, bool strictCheck) {
	...
	if (nonSimVars)
		info("Copied %d non-sim var%s to output config", nonSimVars, (nonSimVars > 1) ? "s" : "");
	if (unused) {
		if (strictCheck) {
...
template <> const char *getTypeName < lc_int64 > () {
	return "uint64";
}

template <> const char *getTypeName < bool > () {
	return "bool";
}

template <> const char *getTypeName < const char *>() {
	return "string";
}
...
template <> SType getSType < int >() {
	return SType::TypeInt;
}

template <> SType getSType < lc_int64 > () {
	return SType::TypeInt64;
}

template <> SType getSType < bool > () {
	return SType::TypeBoolean;
}

template <> SType getSType < const char *>() {
	return SType::TypeString;
}

template <> SType getSType < double >() {
	return SType::TypeFloat;
}

template < typename T > static bool getEq(T v1, T v2);
template <> bool getEq < int >(int v1, int v2) {
	return v1 == v2;
}

template <> bool getEq < lc_int64 > (lc_int64 v1, lc_int64 v2) {
	return v1 == v2;
}

template <> bool getEq < bool > (bool v1, bool v2) {
	return v1 == v2;
}

template <> bool getEq < const char *>(const char *v1, const char *v2) {
	return strcmp(v1, v2) == 0;
}

template <> bool getEq < double >(double v1, double v2) {
	return v1 == v2;
}
...
template < typename T > T Config::genericGet(const char *key, T def) {
	T val;
	if (inCfg->exists(key)) {
...
template < typename T > T Config::genericGet(const char *key) {
	T val;
	if (inCfg->exists(key)) {
		if (!inCfg->lookupValue(key, val)) {
...
template <> uint32_t Config::get < uint32_t > (const char *key) {
	return (uint32_t) genericGet < int >(key);
}

template <> uint64_t Config::get < uint64_t > (const char *key) {
	return (uint64_t) genericGet < lc_int64 > (key);
}

template <> bool Config::get < bool > (const char *key) {
	return genericGet < bool > (key);
}

template <> const char *Config::get < const char *>(const char *key) {
	return genericGet < const char *>(key);
}

template <> double Config::get < double >(const char *key) {
	return (double)genericGet < double >(key);
}

template <> uint32_t Config::get < uint32_t > (const char *key, uint32_t def) {
	return (uint32_t) genericGet < int >(key, (int)def);
}

template <> uint64_t Config::get < uint64_t > (const char *key, uint64_t def) {
	return (uint64_t) genericGet < lc_int64 > (key, (lc_int64) def);
}

template <> bool Config::get < bool > (const char *key, bool def) {
	return genericGet < bool > (key, def);
}

template <> const char *Config::get < const char *>(const char *key, const char *def) {
	return genericGet < const char *>(key, def);
}

template <> double Config::get < double >(const char *key, double def) {
	return (double)genericGet < double >(key, (double)def);
}
...
struct Range {
	...
	explicit Range(string r) {
		...
		for (auto s:t) {
			...
			ss >> x;
			if (ss.fail())
				panic("%s in range %s is not a valid number", s.c_str(), r.c_str());
			n.push_back(x);
		} switch (n.size()) {
		 case 1:
			 min = n[0];
		 ...
		 case 3:
			 ...
			 break;
		 default:
			 panic("Range '%s' can only have 1-3 numbers delimited by ':', %ld parsed", r.c_str(),
			       n.size());
		}

		//Final error-checking
		if (min < 0 || step < 0 || sup < 0)
			panic("Range %s has negative numbers", r.c_str());
		if (step == 0)
			panic("Range %s has 0 step!", r.c_str());
		if (min >= sup)
			panic("Range %s has min >= sup!", r.c_str());
	}

	void fill(vector < bool > &mask) {
		for (int32_t i = min; i < sup; i += step) {
			if (i >= (int32_t) mask.size() || i < 0)
				panic("Range %d:%d:%d includes out-of-bounds %d (mask limit %ld)", min, step, sup, i,
				      mask.size() - 1);
			mask[i] = true;
		}
	}
...
template < typename T > std::vector < T > ParseList(const std::string & listStr, const char *delimiters) {
 ...
 for (auto n:nums) {
		...
		ss >> x;
		if (ss.fail())
			panic("%s in list [%s] could not be parsed", n.c_str(), listStr.c_str());
		res.push_back(x);
	}
	return res;
}
