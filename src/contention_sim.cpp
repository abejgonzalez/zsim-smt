...
bool ContentionSim::CompareEvents::operator() (TimingEvent * lhs, TimingEvent * rhs)
const {
	return lhs->cycle > rhs->cycle;
} bool ContentionSim::CompareDomains::operator() (DomainData * d1, DomainData * d2) const const {
...
ContentionSim::ContentionSim(uint32_t _numDomains, uint32_t _numSimThreads) {
	...
	if ((numDomains % numSimThreads) != 0)
		panic("numDomains(%d) must be a multiple of numSimThreads(%d) for now", numDomains, numSimThreads);
...
void ContentionSim::initStats(AggregateStat * parentStat) {
	...
	for (uint32_t i = 0; i < numDomains; i++) {
		...
		domains[i].profIncomingCrossingHist.init("ixh", "Incoming crossings held count histogram",
							 33 /*32 means >31 */ );
...
void ContentionSim::simulatePhase(uint64_t limit) {
	if (skipContention)
		return;		//fastpath when there are no cores to simulate
...
void ContentionSim::enqueue(TimingEvent * ev, uint64_t cycle) {
	...
	assert_msg(cycle < lastLimit + 10 * zinfo->phaseLength + 1000000,
		   "Queued event too far into the future, cycle %ld lastLimit %ld", cycle, lastLimit);

	assert_msg(cycle >= domains[ev->domain].curCycle, "Queued event goes back in time, cycle %ld curCycle %ld",
		   cycle, domains[ev->domain].curCycle);
	ev->privCycle = cycle;
...
void ContentionSim::enqueueSynced(TimingEvent * ev, uint64_t cycle) {
	...
	assert_msg(cycle >= lastLimit, "Enqueued (synced) event before last limit! cycle %ld min %ld", cycle,
		   lastLimit);
	//Hacky, but helpful to chase events scheduled too far ahead due to bugs (e.g., cycle -1). We should probably formalize this a bit more
	assert_msg(cycle < lastLimit + 10 * zinfo->phaseLength + 10000,
		   "Queued  (synced) event too far into the future, cycle %ld lastLimit %ld", cycle, lastLimit);
	ev->privCycle = cycle;
	assert(ev->numParents == 0);
	domains[ev->domain].pq.enqueue(ev, cycle);
...
void ContentionSim::enqueueCrossing(CrossingEvent * ev, uint64_t cycle, uint32_t srcId, uint32_t srcDomain,
				    uint32_t dstDomain, EventRecorder * evRec) {
...
void ContentionSim::simThreadLoop(uint32_t thid) {
	...
	CPU_SET(thid % nprocs, &cpuset);
	CPU_SET((thid % nprocs) + (nprocs / 2), &cpuset);	//pin to a single core but can use both its hyperthreads, works best (~20% gain)
...
void ContentionSim::simulatePhaseThread(uint32_t thid) {
	...
	if (thDomains == 1) {
		...
		while (pq.size() && pq.firstCycle() < limit) {
			...
			if (newCycle != domCycle)
				domain.curCycle = newCycle;
#if POST_MORTEM
 ...
 for (std::pair < uint64_t, TimingEvent * >p:simThreads[thid].logVec) {
				...
				if (desc == "") {	//non-existnt
					...
					if (ce) {
						ss << " slack " << (ce->preSlack +
								    ce->postSlack) << " osc " << ce->
						    origStartCycle << " cnt " << ce->simCount;
					}
			...
			while (stalledQueue.size()) {
				...
				if (!pq.size() || pq.firstCycle() > limit) {
					...
					if (domain->prio == 0)
						...
						nextStalledQueue.push_back(domain);
				}
				if (domPq.size())
					break;
			}
			if (!stalledQueue.size())
				std::swap(stalledQueue, nextStalledQueue);
		}
	}
